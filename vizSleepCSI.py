...
if True: # plot pose3D
    ...
    def updatefig(i):
        ...
        if(len(SSX)>SSWindowSize):
            ...
            ax0.set_xlim([ SSX[0],SSX[len(SSX)-1] ])
        ax0.plot(SSX,SSY, label='Sleep stage')
            
        csiIndices=sleepIdx2csiIndices_timestamp(sleepIdx,sleepList,csiList,skipframe=skipframe)
        if(len(csiIndices)>0):
            ...
            # print(sleepIndices)
            samplingedAmp,expectedTSs=samplingCSISleep(csiList,csiIndices,sleepList,[sleepIdx,sleepIdx+1],200)
            for j in range(0,52):
                textX=[]
                textY=[]
